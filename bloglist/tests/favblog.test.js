...
describe('Favorite blog', () => {
  test('When the list have only one blog , equals to the blog', () => {
    const result = listHelper.favBlog(listWithOneBlog);
    const { title, author, likes } = listWithOneBlog[0];
    //const filteredList = _.pick(listWithOneBlog, ['title', 'author', 'likes']);
    expect(result).toEqual({ title, author, likes });
  });
  test('when the list have many blog , equals to the blog with most like', () => {
